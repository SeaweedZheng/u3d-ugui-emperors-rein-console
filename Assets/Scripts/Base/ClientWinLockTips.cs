...
public class ClientWinLockTips : MonoSingleton<ClientWinLockTips>
{
    private Text text;
    ...
    {
        if (text.text == str && isShow) return;
        rect.anchoredPosition = new Vector2(360, 313);
        Show(str, time, true);
    }
    ...
    private void Show(string str, float time = 3, bool isSystem = false)
    {
        ...
        if (showTime > -1)
        {
            ...
            isShow = true;
        transform.localScale = isSystem ? Vector3.one * 0.5f : Vector3.one;
    }
