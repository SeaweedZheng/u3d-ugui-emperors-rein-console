...
namespace UnityEngine.UI
{
    ...
    public abstract class LoopScrollRectBase : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        private bool m_neverStopScroll;
        ...
        public int GetLastItem(out float offset)
        {
            ...
            offset = Mathf.Abs(offset) < 1 ? 0 : offset;
            int idx = 0;
            if (itemTypeEnd > itemTypeStart)
            {
        ...
        public void ScrollToCell(int index, float speed)
        {
            ...
            if (speed <= 0)
            {
                Debug.LogErrorFormat("speed <= 0, index {0}", index);
                RefillCells(index);
                return;
            }
        ...
        public void ScrollNeverStop(float speed)
        {
            m_neverStopScroll = true;
            StartCoroutine(ScrollCoroutine(speed));
        }

        public void StopNeverStopScroll()
        {
            m_neverStopScroll = false;
        }

        IEnumerator ScrollCoroutine(float speed)
        {
            while (m_neverStopScroll)
            {
                yield return null;
                if (!m_Dragging)
                {
